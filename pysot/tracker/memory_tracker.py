...
class MemorySiamTracker(SiamRPNTracker):
    def __init__(self, model):
        super(MemorySiamTracker, self).__init__(model)
    ...
    def update_template(self, img, bbox):
        """
        args:
            img(np.ndarray): BGR image
            bbox: (x, y, w, h) bbox
        ...
        # get crop
        z_crop = get_subwindow(img, center_pos,
                                    cfg.TRACK.EXEMPLAR_SIZE,
                                    s_z, channel_average)
        self.model.template_update(z_crop)
